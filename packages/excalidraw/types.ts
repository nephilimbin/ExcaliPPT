...
export interface AppState {
  ...
  // Presentation annotation preferences — sub-state of the laser tool
  // (which "kind" of annotation it draws + shared lifetime / style options).
  currentPresentationKind: AnnotationKind;
  presentationArrowStyle: PresentationArrowStyle;
  presentationDecayMs: number;
  presentationStrokeSize: number;
  presentationColorStrategy: ColorStrategy;
  /** 上次选用的「图形」子形状(矩形/椭圆/三角形),点图形按钮或 W 循环落图形时恢复。全局持久。 */
  lastPresentationShape: "rectangle" | "ellipse" | "triangle";
  /** Presentation text tool: render gate for the annotation text input. Holds
   * no domain data — position/content/font size live in PresentationTrails. */
  isEditingPresentationText: boolean;
...
export type AppClassProperties = {
  ...
  presentationTrails: App["presentationTrails"];
