...
export const textWysiwyg = ({
  ...
  const unbindUpdate = app.scene.onUpdate(() => {
    updateWysiwygStyle();
    // 焦点在属性面板(compact 弹层或 full 顶栏面板)输入框时不抢焦,
    // 否则字号等 NumberInput 的方向键步进会被打断
    const isPanelInputFocused = !!document.activeElement?.closest(
      ".properties-content, .selected-shape-actions-container",
    );
