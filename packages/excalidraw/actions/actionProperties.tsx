...
// 字号滑块/数字输入框的连续调节范围(4 档快选之外的任意字号)
const FONT_SIZE_SLIDER_MIN = 1;
const FONT_SIZE_SLIDER_MAX = 300;
...
export const actionChangeFontSize = register<ExcalidrawTextElement["fontSize"]>(
  {
    ...
    PanelComponent: ({ elements, appState, updateData, app, data }) => {
      ...
      // 数字框:键盘逐字符输入时本地缓冲(避免打"12"中途频繁重算);
      // 上下箭头/方向键/粘贴等非键盘来源则即时提交。
      const [inputValue, setInputValue] = useState(String(fontSize ?? ""));
      useEffect(() => {
        setInputValue(String(fontSize ?? ""));
      }, [fontSize]);
      // true=正在键盘逐字符输入(仅缓冲);false=非键盘来源(即时提交)
        ...
        if (!Number.isNaN(parsed)) {
          applyFontSize(
            Math.min(
              ...
              Math.max(FONT_SIZE_SLIDER_MIN, parsed),
            ),
          );
        }
      };
      const commitInput = () => commitValue(inputValue);
          ...
          <div className="font-size-slider-row">
            ...
            <input
              ...
              onChange={(event) => {
                ...
                setInputValue(value);
                // 非键盘来源(spinner 箭头、粘贴)或已按方向键 → 即时生效
                if (!typingRef.current) {
                  commitValue(value);
                }
              }}
              onMouseDown={() => {
                // 点击 spinner 前重置,确保键入数字后点箭头也即时生效
              ...
              data-testid="fontSize-number-input"
            />
          </div>
        </fieldset>
      );
