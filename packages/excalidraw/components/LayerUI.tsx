...
import { PresentationTextInput } from "./PresentationTextInput";
    ...
    <UIAppStateContext.Provider value={appState}>
      <TunnelsJotaiProvider>
        <TunnelsContext.Provider value={tunnels}>
          {layerUIJSX}
          {appState.isEditingPresentationText &&
            (() => {
              // Position/font size are derived from the in-progress annotation
              // in PresentationTrails (single source of truth) each render —
              // never stored in AppState. Returns null if there is no text
              // annotation being edited (gate briefly out of sync with instance).
              const pos = app.presentationTrails.getEditingTextPos();
              if (!pos) {
                return null;
              }
              const vp = sceneCoordsToViewportCoords(
                { sceneX: pos.x, sceneY: pos.y },
                app.state,
              );
              const rect = canvas.getBoundingClientRect();
              const close = () =>
                setAppState({ isEditingPresentationText: false });
              return (
                <PresentationTextInput
                  ...
                  onChange={(value) =>
                    app.presentationTrails.updateCurrentText(value)
                  }
                  onCommit={() => {
                    app.presentationTrails.commitCurrentText();
                    close();
                  }}
                  onCancel={() => {
                    app.presentationTrails.cancelCurrentText();
                    close();
                  }}
                />
              );
