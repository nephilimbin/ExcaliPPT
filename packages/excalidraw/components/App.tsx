...
import { PresentationTrails } from "../presentation/PresentationTrails";
...
class App extends React.Component<AppProps, AppState> {
  ...
  presentationTrails = new PresentationTrails(this);
  ...
  private handleInteractionStateChange = (
    ...
    if (
      ...
      if (!this.isToolSupported(this.state.activeTool.type)) {
        ...
        this.presentationTrails.endPath();
      }
      this.cursor.reset();
    }
  ...
  private onKeyDown = withBatchedUpdates(
    (event: React.KeyboardEvent | KeyboardEvent) => {
      ...
      if (
        ...
        // W 循环激光笔子种类:画笔 → 直线 → 图形(落 lastPresentationShape)→ 文字 → 画笔
        if (
          event.key === KEYS.W &&
          this.state.activeTool.type === "laser" &&
          !this.state.isEditingPresentationText
        ) {
          const cur = this.state.currentPresentationKind;
          const inShape =
            cur === "rectangle" || cur === "ellipse" || cur === "triangle";
          const next =
            cur === "freehand"
              ? "line"
              : cur === "line"
              ? this.state.lastPresentationShape
              : inShape
              ? "text"
              : "freehand";
          this.setState({ currentPresentationKind: next });
          event.stopPropagation();
          return;
        }
  ...
  private handleCanvasPointerDown = (
    ...
    if (this.state.activeTool.type === "lasso") {
      ...
      if (this.presentationTrails.isEditingText) {
        // clicking elsewhere confirms the current text, then starts a new one
        this.presentationTrails.commitCurrentText();
        this.setState({ isEditingPresentationText: false });
      }
      this.presentationTrails.startPath(
  ...
  private onPointerMoveFromPointerDownHandler(
    ...
    return withBatchedUpdatesThrottled((event: PointerEvent) => {
      ...
      if (this.state.activeTool.type === "laser") {
        this.presentationTrails.addPointToPath(
          pointerCoords.x,
          pointerCoords.y,
        );
      }
  ...
  private onPointerUpFromPointerDownHandler(
    ...
    return withBatchedUpdates((childEvent: PointerEvent) => {
      ...
      if (activeTool.type === "laser") {
        this.presentationTrails.endPath();
        if (this.presentationTrails.isEditingText) {
          const pos = this.presentationTrails.getEditingTextPos();
          if (pos) {
            this.setState({ isEditingPresentationText: true });
          }
        }
        return;
      }
